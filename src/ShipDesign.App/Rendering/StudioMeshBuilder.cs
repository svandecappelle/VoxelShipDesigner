...
/// face either: instead faces are bucketed by (material, occlusion level) and each bucket gets its
/// own pre-darkened brush. Four levels over seven materials is a couple of dozen meshes, which is
/// far cheaper than it sounds and is what makes the crevices read at all.
/// </summary>
public static class StudioMeshBuilder
{
    ...
    public static Result Build(VoxelGrid grid, float voxelSize, ShipParameters p)
    {
        ...
        var solidMeshes = new Dictionary<(VoxelMaterial Material, int Level), MeshGeometry3D>();
        ...
        foreach (var ((material, level), mesh) in solidMeshes)
            solid.Children.Add(new GeometryModel3D(mesh, palette.SolidMaterial(material, level)));
